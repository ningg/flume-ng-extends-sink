	...
	public Status process() throws EventDeliveryException {
		...
		try{
			...
			if ( (event == null) && (rtxInfo.isStarted()) && (!rtxInfo.isFinished()) ) {	// The scenario: only parts of RTX unit are taken.
				String info_rollback = "Only parts of RTX unit are taken, thus we make the TRANSACTION rollback. [Just dance, no affects]. FROM: Ning Guo.";
				logger.info(info_rollback);
				throw new RTXRollbackException(info_rollback);
			}
			
			// publishing is successful. Commit.
			transaction.commit();
		} catch(RTXRollbackException ex){
			transaction.rollback();
		} catch (Exception ex){
			transaction.rollback();
			String errorMsg = "Failed to publish event: " + event ;
			if (rtxInfo.isStarted()) {
				errorMsg = errorMsg + ". RTXInfo is: " + gson.toJson(rtxInfo);
			}
